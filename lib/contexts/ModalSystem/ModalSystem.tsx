...
const ModalSystemProvider = ({ children }: { children: ReactNode }) => {
  const [modalSystem, dispatchModalAction] = useReducer(
    modalSystemReducer,
    initialModalSystem,
  );
  return (
    <ModalContextObject.Provider
      value={{ system: modalSystem, dispatch: dispatchModalAction }}
    >
      {children}
    </ModalContextObject.Provider>
  );
